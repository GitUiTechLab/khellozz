...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.addPlayer.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.add-players.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('father_mother_name') ? 'is-invalid' : '' }}" type="text" name="father_mother_name" id="father_mother_name" value="{{ old('father_mother_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('event_title') ? 'is-invalid' : '' }}" name="event_title_id" id="event_title_id" required>
                    @foreach($event_titles as $id => $entry)
                        <option value="{{ $id }}" {{ old('event_title_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
